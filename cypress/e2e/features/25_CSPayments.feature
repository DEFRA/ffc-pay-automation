...
# npm run cypress:local:one -- "cypress\e2e\features\25_CSPayments.feature"
  ...
  Scenario: 01 insert incorrect CS test data via service bus message to ffc-pay-request
    ...
    When I send "cs error" test data message to the service bus topic "ffc-pay-request-dev"
  ...
  #without the risk of data conflicts  
    ...
    When I send "cs payment" test data message to the service bus topic "ffc-pay-request-dev"
  ...
  #Updates template values with values used in payment message  
    ...
    When I send "cs return" test data message to the service bus topic "ffc-pay-return-dev"
    Then I confirm that "return" test data in dev has been inserted into ffc-pay-processing database

  #Updates template values with values used in payment message

    When I send "cs ppa" test data message to the service bus topic "ffc-pay-request-dev"
  ...
  #from payment file  
    ...
    Then I take a screenshot for Feature 25 and Scenario 2
 ...
 #First ensure that incorrect data will not be processed
    ...
    Given I visit the "Request Editor" homepage
    Then I take a screenshot for Feature 25 and Scenario 1
    When I send the updated "csError-paymentFileMessage" message to the service bus topic "ffc-pay-request-auto"
    Then I confirm that payment test data has not been inserted into the ffc-pay-processing database

  @local
  Scenario: 02 insert test data via service bus message to ffc-pay-request

    When I send the updated "cs-paymentFileMessage" message to the service bus topic "ffc-pay-request-auto"
...
#This scenario confirms that a return file message can be sent and processed correctly
    ...
    When I send the updated "cs-returnFileMessage" message to the service bus topic "ffc-pay-return-auto"
  ...
  #This scenario confirms that a PPA file message can be sent and processed correctly
    ...
    When I send the updated "cs-ppaFileMessage" message to the service bus topic "ffc-pay-request-auto"
...
#This scenario confirms that payment can be approved from the quality check queue in Request Editor and that E2E journey is complete
  ...
  Scenario: 08 Confirm payment request processed in Payment Management
    ...
    And I select "CS" from the monitor schemes dropdown
    And I click on the "Continue" button
    Then I confirm that payment for "CS" scheme with "3" payment installments totalling "£10,000.00" is displayed
    Then I take a screenshot for Feature 25 and Scenario 8