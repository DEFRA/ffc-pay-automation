Feature: 26 BPS Payments

# npm run cypress:dev:one -- "cypress\e2e\features\26_BPSPayments.feature"
# npm run cypress:local:one -- "cypress\e2e\features\26_BPSPayments.feature"

# This feature file is designed to test the end-to-end journey of BPS payment in the local environment.

  @dev
  Scenario: 01 insert incorrect BPS test data via service bus message to ffc-pay-request

    When I send "bps error" test data message to the service bus topic "ffc-pay-request-dev"
  ...
  #without the risk of data conflicts  
    ...
    When I send "bps payment" test data message to the service bus topic "ffc-pay-request-dev"
    ...
    Then I pull bps payments file from Azure Blob Storage and confirm that correct values have been generated

  #Updates template values with values used in payment message  

    When I send "bps return" test data message to the service bus topic "ffc-pay-return-dev"
    Then I confirm that "return" test data in dev has been inserted into ffc-pay-processing database

  #Updates template values with values used in payment message

    When I send "bps ppa" test data message to the service bus topic "ffc-pay-request-dev"
  ...
  #from payment file  
    ...
    Then I take a screenshot for Feature 26 and Scenario 2
 ...
 #First ensure that incorrect data will not be processed
    ...
    Given I visit the "Request Editor" homepage
    Then I take a screenshot for Feature 26 and Scenario 1
    When I send the updated "bpsError-paymentFileMessage" message to the service bus topic "ffc-pay-request-auto"
    Then I confirm that payment test data has not been inserted into the ffc-pay-processing database

  @local
  Scenario: 02 insert test data via service bus message to ffc-pay-request

    When I send the updated "bps-paymentFileMessage" message to the service bus topic "ffc-pay-request-auto"
...
#This scenario confirms that a return file message can be sent and processed correctly
    ...
    When I send the updated "bps-returnFileMessage" message to the service bus topic "ffc-pay-return-auto"
  ...
  #This scenario confirms that a PPA file message can be sent and processed correctly
    ...
    When I send the updated "bps-ppaFileMessage" message to the service bus topic "ffc-pay-request-auto"
...
#This scenario confirms that payment can be approved from the quality check queue in Request Editor and that E2E journey is complete
    ...
    And I click on the "Review" link
    And I click on the "Yes" radio button
    Then I take a screenshot for Feature 26 and Scenario 7
    And I click on the "Submit" button
  ...
  Scenario: 08 Confirm payment request processed in Payment Management
    ...
    Then I take a screenshot for Feature 26 and Scenario 8