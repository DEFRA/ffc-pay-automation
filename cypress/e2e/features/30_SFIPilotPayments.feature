...
# This feature file is designed to test the end-to-end journey of SFI Pilot in the local environment.
  ...
  Scenario: 01 insert incorrect SFI Pilot test data via service bus message to ffc-pay-request
    ...
    When I send "sfi pilot error" test data message to the service bus topic "ffc-pay-request-dev"
  ...
  #without the risk of data conflicts  
    ...
    When I send "sfi pilot payment" test data message to the service bus topic "ffc-pay-request-dev"
    ...
    Then I pull sfi pilot payments file from Azure Blob Storage and confirm that correct values have been generated

  #Updates template values with values used in payment message  

    When I send "sfi pilot return" test data message to the service bus topic "ffc-pay-return-dev"
    Then I confirm that "return" test data in dev has been inserted into ffc-pay-processing database

  #Updates template values with values used in payment message

    When I send "sfi pilot ppa" test data message to the service bus topic "ffc-pay-request-dev"
 ...
 #First ensure that incorrect data will not be processed
    ...
    When I send the updated "sfiPilotError-paymentFileMessage" message to the service bus topic "ffc-pay-request-aw"
    Then I confirm that payment test data has not been inserted into the ffc-pay-processing database

  @local
  Scenario: 02 insert test data via service bus message to ffc-pay-request

    When I send the updated "sfiPilot-paymentFileMessage" message to the service bus topic "ffc-pay-request-aw"
...
#This scenario confirms that a return file message can be sent and processed correctly
    ...
    When I send the updated "sfiPilot-returnFileMessage" message to the service bus topic "ffc-pay-return-aw"
    Then I confirm that "return" test data has been inserted into the "ffc-pay-processing" database

  @local
  Scenario: 04 send SFI Pilot PPA file message and confirm processing

  #This scenario confirms that a PPA file message can be sent and processed correctly

    When I send the updated "sfiPilot-ppaFileMessage" message to the service bus topic "ffc-pay-request-aw"
